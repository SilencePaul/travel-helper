...
test("the trip navigation exposes the authenticated decision workspace route", async () => {
  ...
  render(<MemoryRouter initialEntries={["/decisions"]}><TripApp repository={new LocalTripRepository(seed)} decisionRepository={decisionRepository} agentBridge={agentBridge} member={member} /></MemoryRouter>);
  ...
  expect(screen.getByRole("button", { name: "准备本机 Codex" })).toBeVisible();
});
