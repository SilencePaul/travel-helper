  ...
  return (
    <article className="hotel-card" data-selected={selected || undefined}>
      ...
      <dl className="hotel-facts">
        <div><dt>住宿 {nights} 晚（含税参考总额）</dt><dd>{hotel.nightlyPrice.currency} {hotel.stayTotal.toFixed(2)}</dd></div>
        <div><dt>每晚含税参考价</dt><dd>{hotel.nightlyPrice.currency} {hotel.nightlyPrice.amount.toFixed(2)}</dd></div>
