...
export function HotelCard({ hotel, nights, selected, onSelect }: HotelCardProps) {
  return (
    <article className="hotel-card" data-selected={selected || undefined}>
      ...
      <p className="hotel-source">原始分数：价格 {hotel.priceScore.toFixed(3)} · 通勤 {hotel.commuteScore.toFixed(3)}；通勤 {hotel.totalCommuteMinutes} 分钟 / {hotel.estimatedSteps.toLocaleString()} 步</p>
