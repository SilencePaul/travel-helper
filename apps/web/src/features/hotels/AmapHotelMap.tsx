...
export function AmapHotelMap({ hotels, selectedId, onSelect, mapLoader = loadAmap }: Props) {
  ...
  useEffect(() => {
    ...
    void mapLoader().then((AMap) => {
      ...
      hotels.forEach((hotel) => {
        ...
        const marker = new AMap.Marker({ map, position: [hotel.coordinate.lng, hotel.coordinate.lat], title: hotel.name, zIndex: selected ? 200 : 100, content: `<span class="hotel-amap-marker${selected ? " is-selected" : ""}"></span>` });
  ...
  useEffect(() => { const hotel = hotels.find((item) => item.id === selectedId); if (hotel) { mapRef.current?.setCenter([hotel.coordinate.lng, hotel.coordinate.lat]); Object.entries(markersRef.current).forEach(([id, marker]) => { marker.setContent?.(`<span class="hotel-amap-marker${id === selectedId ? " is-selected" : ""}"></span>`); marker.setzIndex?.(id === selectedId ? 200 : 100); }); } }, [hotels, selectedId]);
