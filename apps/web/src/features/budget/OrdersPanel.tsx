    ...
    <section className="orders-panel" aria-labelledby="orders-heading">
      ...
      {orders.length === 0 ? <p className="empty-state">尚未录入订单；酒店候选价格仅作比较，不会被当作已预订或已付款。</p> : <ul className="order-list">
        {orders.map((order) => <li key={order.id}>
          ...
          <label>{order.name}已付金额<input aria-label={`${order.name}已付金额`} type="number" min="0" step="0.01" disabled={disabled || !onPaidChange || savingId === order.id} value={drafts[order.id] ?? (order.paid / 100).toFixed(2)} onChange={(event) => { const draft = event.target.value; setDrafts((current) => ({ ...current, [order.id]: draft })); const isClean = /^\d+(?:\.\d{1,2})?$/.test(draft) && Math.round(Number(draft) * 100) === order.paid; setDirtyIds((current) => { if (isClean && !current.has(order.id)) return current; if (!isClean && current.has(order.id)) return current; const next = new Set(current); if (isClean) next.delete(order.id); else next.add(order.id); return next; }); if (isClean) setError(undefined); }} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void savePaid(order); } }} /></label>
          <button aria-label={`保存 ${order.name} 金额`} type="button" disabled={disabled || !onPaidChange || savingId === order.id} onClick={() => void savePaid(order)}>{savingId === order.id ? "正在保存" : "保存金额"}</button>
