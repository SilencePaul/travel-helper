...
export function DecisionWorkspacePage({ repository, trip, member, onBack, newIdempotencyKey = defaultIdempotencyKey }: Props) {
  ...
  if (!workspace) return <main className="decision-page"><button className="control-button control-button--text" type="button" onClick={onBack}>返回行程</button><p role={message.includes("无法") ? "alert" : "status"}>{message}</p></main>;
  ...
  return <main className="decision-page">
    ...
    <form className="decision-preference-form" key={profile?.revision ?? 0} aria-busy={busy} onSubmit={savePreference}>
      ...
      <div className="decision-form-actions">
        ...
        <button className="control-button control-button--secondary" type="button" disabled={busy || !profile} onClick={() => profile && void runCommand({ action: "completePreference", tripId: trip.id, expectedRevision: profile.revision, idempotencyKey: newIdempotencyKey() })}>标记已完成</button>
        <button className="control-button control-button--secondary" type="button" disabled={busy || !profile} onClick={() => profile && void runCommand({ action: "skipPreference", tripId: trip.id, expectedRevision: profile.revision, idempotencyKey: newIdempotencyKey() })}>本轮跳过</button>
        <button className="control-button control-button--secondary" type="button" disabled={busy || !canGenerateSummary} onClick={() => void runCommand({ action: "generatePreferenceSummary", tripId: trip.id, sourcePreferenceRevisions: Object.fromEntries(finishedProfiles.map(({ ownerUid, revision }) => [ownerUid, revision])), idempotencyKey: newIdempotencyKey() })}>生成共同摘要</button>
      </div>
    </form>

    {message ? <p className="decision-page-message" role={message.includes("失败") || message.includes("更新") ? "alert" : "status"}>{message}</p> : null}
    ...
    {workspace.candidates.length > 0 ? <section className="decision-action-desk" aria-labelledby="decision-actions-title" aria-busy={busy}>
      ...
      {workspace.candidates.slice(0, 4).map((candidate) => {
        ...
        return <article key={candidate.id} className="decision-action-row">
          ...
          <button className="control-button control-button--secondary" type="button" disabled={busy} onClick={() => void runCommand({ action: "recordFeedback", tripId: trip.id, candidateId: candidate.id, kind: "like", idempotencyKey: newIdempotencyKey() })}>喜欢</button>
          <button className="control-button control-button--danger" type="button" disabled={busy} onClick={() => void runCommand({ action: "recordFeedback", tripId: trip.id, candidateId: candidate.id, kind: "dislike", idempotencyKey: newIdempotencyKey() })}>反对</button>
          <button className="control-button control-button--primary" type="button" disabled={busy || !firstDay || candidate.decisionState === "confirmed"} onClick={() => firstDay && void runCommand({ action: "placeTentative", tripId: trip.id, candidateId: candidate.id, expectedCandidateRevision: candidate.revision, placement: { tripDayId: firstDay.id, date: firstDay.date, sortKey: `candidate-${candidate.id}` }, idempotencyKey: newIdempotencyKey() })}>{placement ? "调整暂定日程" : "加入暂定行程"}</button>
          <button className={`control-button ${ownReceipt?.active ? "control-button--danger" : "control-button--primary"}`} type="button" disabled={busy || !placement || candidate.decisionState === "none"} onClick={() => void runCommand({ action: "setConfirmationReceipt", tripId: trip.id, candidateId: candidate.id, expectedCandidateRevision: candidate.revision, active: !ownReceipt?.active, idempotencyKey: newIdempotencyKey() })}>{ownReceipt?.active ? "撤回我的确认" : "确认这张票根"}</button>
        </article>;
      })}
    </section> : null}
  </main>;
